...
import PrepareAndroid from './Introduction.BuildingWithDetox.AppConfigs.mdx';
...
So, Detox generated those three files in your project's root:
...
Yet, your Detox config is still not ready – you need to provide it the right **commands
to build** your app for iOS and Android, and also the right **paths to the app binaries** so
that Detox can install them on the device and launch for your tests.
...
+      build: 'xcodebuild -workspace ios/example.xcworkspace -scheme example -configuration ${conf} -sdk iphonesimulator -derivedDataPath ios/build'
    ...
    <TabItem value="Android" label="Android">
        <PrepareAndroid
            ...
            appDiff={(conf) => `\
   apps: {
     'android.${conf.toLowerCase()}': {
       type: 'android.apk',
-      binaryPath: 'android/app/build/outputs/apk/${conf.toLowerCase()}/app-${conf.toLowerCase()}.apk',
+      binaryPath: 'android/app/build/outputs/apk/${conf.toLowerCase()}/app-${conf.toLowerCase()}.apk',
-      build: 'cd android && ./gradlew assemble${conf} assembleAndroidTest -DtestBuildType=${conf.toLowerCase()}'
+      build: 'cd android && ./gradlew assemble${conf} assembleAndroidTest -DtestBuildType=${conf.toLowerCase()}'
     },
            `}/>
        <div>
